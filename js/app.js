(() => {
    ...
    da.init();
    let boxsStars = document.querySelectorAll(".reviews__slide_stars");
    if (boxsStars) for (let index = 0; index < boxsStars.length; index++) {
        const boxStars = boxsStars[index];
        let star = boxStars.querySelector(".star");
        let quantityBoxStars = boxStars.dataset.quantity - 1;
        let stroka = star.parentNode.innerHTML;
        let rezult = stroka.repeat(quantityBoxStars);
        boxStars.insertAdjacentHTML("afterbegin", `${rezult}`);
    }
    window["FLS"] = false;
    menuInit();
    menuClose();
